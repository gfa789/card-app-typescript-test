...
export default function AllEntries(){
    ...
    if(entries.length == 0){
        return(
            <section>
                <h1 className="text-center font-semibold dark:text-white text-2xl m-5">You don't have any card</h1>
                <p className="text-center font-medium dark:text-white text-md">Lets <Link className="text-blue-400 underline underline-offset-1" to="/create">Create One</Link></p>
            </section>
        )
    }
    return(
        <section className={`${darkMode ? "" : ""}grid grid-cols-2 md:grid-cols-4`}>
            {entries.map((entry: Entry, index: number) => {
                return(
                    <div id={entry.id} key={index}className={`${darkMode ? 'bg-gray-600 text-white ': 'bg-gray-300 '} shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col justify-between`}>
